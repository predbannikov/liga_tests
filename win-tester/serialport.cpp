...
void SerialPort::parseReqest(QJsonObject &jobj)
{
    if (jobj["action"].toString() == "usually") {
        port->clear(QSerialPort::AllDirections);
        if (!port->waitForReadyRead(-1)) {        // таймоут или ошибка
...
//        modbus_call(buff);
        ...
        jobj["PDU_request"] = QString(buff.toHex());
        jobj["address"] = QString::number(buff[0]);
...
//        quint8 address = jobj["address"].toString().toInt();
        ...
        quint8 address = buff[0];
//        buff.remove(0, 1);
        buff.remove(buff.size() - 2, 2);

        parsePDU(buff);
//        quint16 crc = calculateCRC16(buff);
//        buff.append()
//        buff.append(2, *reinterpret_cast<char*>(&crc));
        QDataStream str(&buff, QIODevice::ReadWrite | QIODevice::Append);
        str.setVersion(QDataStream::Qt_5_11);
        str.setByteOrder(QDataStream::LittleEndian);
        str << calculateCRC16(buff);
//        buff << qint16(crc);
        jobj["PDU_response"] = QString(buff.toHex());
        port->write(buff);
...
void SerialPort::parsePDU(QByteArray &buff)
{
//    static enum STATE_PDU {STATE_PDU_03 = 0x03, STATE_PDU_10 = 0x10} state = STATE_PDU_03;
    switch (buff[1]) {
    case 0x03:
        liga.read(buff);
        break;
